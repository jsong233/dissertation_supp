...
void Cadherin::gen_c(vector<vector<double> >& X, GCpara gc_para, vector<vector<vector<vector<double> > > >& Sc, vector<vector<int> >& nc, vector<vector<vector<double> > >& Tc, vector<int>& LB){
  ...
  int N = gc_para.N; int Nc_max = gc_para.Nc_max;
  vector<int> Nc(N, 0); Nc = gc_para.Nc;
  double r = gc_para.r; double Lc = gc_para.Lc;
  vector<double> argC(N, 0); argC = gc_para.argC;
  vector<double> rangeC(N, 0); rangeC = gc_para.rangeC;
  ...
  for (int i = 0; i < N; i++){
      for (int j = 0; j < N; j++){
          ...
          if (j != i && d <= 2 * r + 2 * Lc){
              for (int k = 0; k < Nc[i]; k++){  // Nc[i] attempts for cell i
                double br = 0;
                if ((LB[i] == 1 && LB[j] == 0) || (LB[i] == 0 && LB[j] == 1)){
                    std::uniform_real_distribution<> Udistr(r, r + Lc); // 2 * Lc changed on Jun05
                    br = Udistr(generator);
                }
                else{
                ...
                double b[2] = {br * cos(btheta), br * sin(btheta)}; // random reach-out vector
                double delta[2];
                for (int l = 0; l < 2; l++){
                    delta[l] = X[j][l] - X[i][l];
                }
                ...
                double norm_b = 0; double norm_delta = 0;
                for (int l = 0; l < 2; l++){
                    dot_b_delta += b[l] * delta[l];
                    norm_b += b[l] * b[l];
                    norm_delta += delta[l] * delta[l];
                }
                norm_b = sqrt(norm_b); norm_delta = sqrt(norm_delta);
                
                // if the angle is less than pi/8
                if (dot_b_delta/(norm_b * norm_delta) >= cos(pi/8)){ // change to cos(pi/8) on Jul29
                    nnc[i][j] += 1; // add one c-site between i,j
                    int position = nnc[i][j]-1; // the k-th c-site locate at k-1
                    for (int l = 0; l < 2; l++){
                        sc[i][j][position][l] = X[i][l] + b[l];
                    }
                }
              } // end of for k
          } // end of if
...
void Cadherin::gen_c_init(vector<vector<double> >& X, GCpara gc_para, vector<vector<vector<vector<double> > > >& Sc, vector<vector<int> >& nc, vector<vector<vector<double> > >& Tc, vector<int>& LB){
  ...
  int N = gc_para.N; int Nc_max = gc_para.Nc_max; Nc_max = Nc_max / 2;
  vector<int> Nc(N, 0); Nc = gc_para.Nc; 
      ...
      for (int j = 0; j < N; j++){
          ...
          if (j != i && d <= 2 * r + 2 * Lc){
              for (int k = 0; k < Nc[i]; k++){
                double br = 0;
                if ((LB[i] == 1 && LB[j] == 0) || (LB[i] == 0 && LB[j] == 1)){
                    std::uniform_real_distribution<> Udistr(r, r + Lc); //  2 * Lc
                    br = Udistr(generator);
                }
                else{
                    std::uniform_real_distribution<> Udistr(r, r + Lc);
                ...
                // if the angle is less than pi/4
                if (dot_b_delta/(norm_b * norm_delta) >= cos(pi/8)){  // change to cos(pi/8) on Jul29
                    nnc[i][j] += 1; // add one c-site between i,j
                    int position = nnc[i][j]-1; // the k-th c-site locate at k-1
                    for (int l = 0; l < 2; l++){
...
void Cadherin::gen_c_single(vector<vector<double> >& X, int tba_i, int tba_j, GCpara gc_para, double* Sc, double& Tc, vector<int>& LB){
    ...
    if (check0 == 0 && check1 != 0){
        Sc[0] = sc[1][0]; Sc[1] = sc[1][1];
    }
    else if (check0 != 0 && check1 == 0){
        Sc[0] = sc[0][0]; Sc[1] = sc[0][1];
    }
    else if (check0 != 0 && check1 != 0){
        vector<int> weights = {1,1};
        std::discrete_distribution<> d(weights.begin(), weights.end());
        int target = d(generator); 
        Sc[0] = sc[target][0]; Sc[1] = sc[target][1];
    }
...
void Cadherin::removeC(vector<vector<vector<vector<double> > > >& Sc, vector<vector<int> >& nc, vector<vector<vector<double> > >& Tc, int* idx, int N, int Nc){
    int i = idx[0]; int j = idx[1]; int k = idx[2];
    ...
    nSc = Sc; nnc = nc; nTc = Tc;

    // update nnc
    nnc[i][j] = nc[i][j] - 1; nnc[j][i] = nc[j][i] - 1;
    ...
    nSc[i][j][Nc-1][0] = 0; nSc[i][j][Nc-1][1] = 0;
    nSc[j][i][Nc-1][0] = 0; nSc[j][i][Nc-1][1] = 0;
    ...
    nTc[i][j][Nc-1] = 0; nTc[j][i][Nc-1] = 0;

    // substitute back
    Sc = nSc; nc = nnc; Tc = nTc;
}
...
void Cadherin::addC(vector<vector<vector<vector<double> > > >& Sc, vector<vector<int> >& nc, vector<vector<vector<double> > >& Tc, int* cidx, double* ccor, double ctime, int N, int Nc){
    ...
    if (nc[i][j] != Nc){
        int position = nc[i][j];
        nSc[i][j][position][0] = ccor[0]; nSc[j][i][position][0] = ccor[0];
        nSc[i][j][position][1] = ccor[1]; nSc[j][i][position][1] = ccor[1];
        nTc[i][j][position] = ctime; nTc[j][i][position] = ctime;
        nnc[i][j] = nc[i][j] + 1; nnc[j][i] = nc[j][i] + 1;
    }
...
void Integrin::gen_i(vector<vector<double> >& X, GIpara gi_para, vector<vector<vector<double> > >& Si, vector<int>& ni, vector<vector<double> >& Ti, vector<int>& LB){
  ...
  int N = gi_para.N; int Ni = gi_para.Ni;
  double r = gi_para.r; double Li = gi_para.Li;
  vector<double> argC(N, 0); argC = gi_para.argC; 
  double ti_PST = gi_para.ti_PST; double ti_PSP = gi_para.ti_PSP;
            ...
            //   }
              for (int k = 0; k < Ni; k++){
                  ...
                  if (len_i <= r + Li){
                        ...
                        int pos = ni[i] - 1;
                        if (argC[i] > -pi/2 && argC[i] < pi/2){ // point to the right
...
void Integrin::gen_i_init(vector<vector<double> >& X, GIpara gi_para, vector<vector<vector<double> > >& Si, vector<int>& ni, vector<vector<double> >& Ti, vector<int>& LB){
  ...
  int N = gi_para.N; int Ni = gi_para.Ni; Ni = Ni / 2; 
  double r = gi_para.r; double Li = gi_para.Li;
  vector<double> argC(N, 0); argC = gi_para.argC; 
  double ti_PST = gi_para.ti_PST; double ti_PSP = gi_para.ti_PSP;
...
void Integrin::addI(vector<vector<vector<double> > >& Si, vector<int>& ni, vector<vector<double> >& Ti, int idx, double* icor, double itime, int N, int Ni){
    ...
    if (ni[i] != Ni){
        int pos = ni[i];
        nSi[i][pos][0] = icor[0]; nSi[i][pos][1] = icor[1];
        nTi[i][pos] = itime; nni[i] = ni[i] + 1;
    }
