...
void f(int nij, double alpha, double r, double lc, double Lc, double *x, vector<vector<double>> &s, vector<vector<double>> &F);
void f1(double alpha, double r, double lc, double Lc, double *x, double *y, double *F);
    ...
    for (int i = 0; i < N; i++)
    {
        for (int j = 0; j < N; j++)
        {
            ...
            { // if there exists c-sites between i and j
                ...
                f(nc_ij, alphaC, r, lc, Lc, Vi, Sc, Fc);
                for (int l = 0; l < 2; l++)
                {
    ...
    for (int i = 0; i < N; i++)
    {
        for (int j = i + 1; j < N; j++)
        {
            ...
            for (int k = startP; k < endP; k++)
            {
                ...
                double Fi[2];
                double Fj[2];
                f1(alphaC_1, r, lc, Lc, Vi, Vk, Fi);
                f1(alphaC_2, r, lc, Lc, Vj, Vk, Fj);
                for (int l = 0; l < 2; l++)
                {
