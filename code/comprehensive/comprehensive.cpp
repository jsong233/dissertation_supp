...
double r = 1;                        // radius of the nucleus
double lc = 2;                       // natural length of springs for cadherins
double li = 1;                       // natural length of springs for integrins
double Lc = 8;                       // max length of the springs for cadherins
double nl = 2;
double Li = 7;                       // max length of the springs for integrins
double argC = 0;                     // angle of the cAMP gradient [-pi,pi]
double rangeC = pi / 16;             // reach out range
int N = 48;                          // total # of cells
int rowN = 12;                       // # of cells in each row
int columnN = 4;                     // # of cells in each column
double gap = 2 * r;
double margin_x = 10 * r;
double margin_y = 0 * r;
int Nc = 4;                          // max # of cadherins formed between two cells
int mNc = 2;                         // min # of cadherins formed between two cells
int Ni = 4;                          // max # of integrins one cell can form
int mNi = 1;                         // min # of integrins of a cell
vector<double> AlphaC{350,350};      // spring constant of cadherins 
vector<double> AlphaI{0,350};        // spring constant of integrins 
double alphaB = 2000;                // spring constant for cell-cell body force 
vector<double> AlphaM = {50,50,50};  // membrane body force parameters, TT, PP, TP
double alphaS = 2500;                // spring constant for cell-substrate body force 
double vis_ratio = 10;
double mu = 0.101/60;                // viscosity of cells 
double nu = mu / vis_ratio;          // viscosity of cadherins 
double tc_TT = 3;                    // attach time for each cadherin
double tc_PP = 3;
double tc_TP = 3;
double ti_PSP = 0.1;                 // attach time for each integrin
double ti_PST = 0.1;
...
double reltol = 1e-6;
double abstol = 1e-6;
int maxstep = 10000;                 // max steps taken for the ode solver
...
double chaL = 5.5 * r;                          // characteristic dimension
int mx = Nx + 1; int my = Ny + 1;
double dx = Lx / Nx; double dy = Ly / Ny;       // length of subintervals
// # of matrix entries in the linear system
static const long long int MAX_ENTRY = 89 * 25 * 89 * 25;  
// dimension of rhs in the linear equation
static const int MAX_DIM = 89 * 25;             
static const int NX = 88; static const int NY = 24;
double A_longarray[MAX_ENTRY] = {0};
double rhs_longarray[MAX_DIM] = {0};
float Dx[NX-1] = {0}; float Dy[NY-1] = {0};
float Bx[NX-2] = {0}; float By[NY-2] = {0};
...
double T_ct = 2; double T_cp = 2;    // period of camp output
double T_dt = 2; double T_dp = 2;    // period of dif output
double base_ct = 0.099 / iPDE;       // basal level of PST outputting camp
...
double slop_ct = 0;                  // slope of PST outputting camp (/min)
...
double V0 = (Lx * chaL) * (Ly * chaL) * chaL;  // volume of the extracellular medium
double Vc = 696.9;                             // volume of a cell (micrometers^3)
double gm6_ct = 11.6; double gm6_cp = 11.6; 
double gm6_dt = 1.16; double gm6_dp = 1.16; 
double gm7_ct = 0; double gm7_cp = 0;          // no local degradation 
double gm7_dt = gm7_cp; double gm7_dp = 0;     // no local degradation 
double gm8_c = 0.1; double gm8_d = gm8_c; 
double gm9_c = 0.1; double gm9_d = gm9_c;      // small global degradation
double gm9_hat_c = gm9_c * N * K_w * Vc / V0; 
double gm9_hat_d = gm9_hat_c; 
...
double exp_c = 1; double exp_d = 1;  // exponents in transition functions 
double int_fdb_t = 1;                // feedback 1 minute prior to differentiation
...
int main()
{
    ...
    PDEpara PCpara; PDEpara PDpara;
    NLpara FCpara; NLpara FDpara;
    Cadherin cad;
    Integrin intg;
    SUNLinearSolver LS;

    // parameters for generating cadherins
    gc_para.N = N;
    gc_para.Nc = Nc;
    gc_para.r = r; gc_para.Lc = Lc;
    gc_para.tc_PP = tc_PP; 
    gc_para.tc_TT = tc_TT;
    gc_para.tc_TP = tc_TP;

    // parameters for generating integrins
    gi_para.N = N; gi_para.Ni = Ni;
    gi_para.r = r; gi_para.Li = Li;
    gi_para.ti_PSP = ti_PSP; gi_para.ti_PST = ti_PST;

    // parameters for motion equations
    motion_para.N = N; motion_para.Ni = Ni;
    motion_para.r = r; motion_para.lc = lc; motion_para.Lc = Lc; 
    motion_para.nl = nl;
    motion_para.li = li; motion_para.Li = Li;
    motion_para.AlphaC = AlphaC; motion_para.AlphaI = AlphaI;
    motion_para.alphaB = alphaB; motion_para.alphaS = alphaS;
    motion_para.AlphaM = AlphaM;
    motion_para.mu = mu; motion_para.nu = nu;

    // parameters for cAMP pde solver
    PCpara.N = N; PCpara.Lx = Lx; PCpara.Ly = Ly;
    PCpara.Nx = Nx; PCpara.Ny = Ny;
    PCpara.mindt = mindt; PCpara.sigma = sigma_c;
    PCpara.tol = CKtol;
    PCpara.baseT = base_ct; PCpara.baseP = base_cp; 
    PCpara.tshT = tsh_ct; PCpara.tshP = tsh_cp;
    PCpara.Tt = T_ct; PCpara.Tp = T_cp;
    PCpara.slopT = slop_ct; PCpara.slopP = slop_cp;

    // parameters for DIF pde solver
    PDpara.N = N; PDpara.Lx = Lx; PDpara.Ly = Ly;
    PDpara.Nx = Nx; PDpara.Ny = Ny;
    PDpara.mindt = mindt; PDpara.sigma = sigma_d;
    PDpara.tol = CKtol;
    PDpara.baseT = base_dt; PDpara.baseP = base_dp; 
    PDpara.tshT = tsh_dt; PDpara.tshP = tsh_dp;
    PDpara.Tt = T_dt; PDpara.Tp = T_dp;
    PDpara.slopT = slop_dt; PDpara.slopP = slop_dp;

    // parameters for nonlinear term in cAMP pde
    FCpara.N = N; FCpara.Nx = Nx; FCpara.Ny = Ny;
    FCpara.K = K_w; FCpara.Lx = Lx; FCpara.Ly = Ly;
    FCpara.Vc = Vc; FCpara.V0 = V0;
    FCpara.gm6_t = gm6_ct; FCpara.gm6_p = gm6_cp; 
    FCpara.gm7_t = gm7_ct; FCpara.gm7_p = gm7_cp;
    FCpara.gm8 = gm8_c; FCpara.gm9 = gm9_hat_c;

    // parameters for nonlinear term in DIF pde
    FDpara.N = N; FDpara.Nx = Nx; FDpara.Ny = Ny;
    FDpara.K = K_w; FDpara.Lx = Lx; FDpara.Ly = Ly;
    FDpara.Vc = Vc; FDpara.V0 = V0;
    // FDpara.src = src_d; FDpara.src_loc = src_loc_d;
    FDpara.gm6_t = gm6_dt; FDpara.gm6_p = gm6_dp; 
    FDpara.gm7_t = gm7_dt; FDpara.gm7_p = gm7_dp;
    FDpara.gm8 = gm8_d; FDpara.gm9 = gm9_hat_d;

    // grid points including boundaries
    vector<double> x(mx, 0); vector<double> y(my, 0);
    for (int j = 0; j < mx; j++){
        x[j] = j * dx;
    }
    for (int k = 0; k < my; k++){
    ...
    /*****************************************************************
                                Time Tables                     
    ******************************************************************/
    vector<double> Tgc = {}; vector<double> Tgi = {};
    double cumultimeC = 0; double cumultimeI = 0;
    int Tgc_len = 0; int Tgi_len = 0;
    ...
    // time table for the system to generate new integrins
    while (cumultimeI < T_MAX){
        std::normal_distribution<> NdistrI(0.01, 0.005);
        double gi_interval = NdistrI(generator);
        if (gi_interval > 0){
            cumultimeI += gi_interval;
            Tgi.push_back(cumultimeI);
            Tgi_len += 1;
        }
    } 
    ...
    ******************************************************************/
    // create output files ------------------------------------------
    string filename1 = "motion.dat"; ofstream *foutm = new ofstream(filename1);
    string filename2 = "dif.dat"; ofstream *foutd = new ofstream(filename2);
    string filename3 = "camp.dat"; ofstream *foutc = new ofstream(filename3);
    ...
    for (int i = 0; i < N; i++){
        ...
        int posy = i / rowN;
        X[i][0] = r + margin_x + posx * gap; X[i][1] = r + margin_y + posy * gap;
    }
    ...
    vector<double> uc(mx * my, 0); vector<double> ud(mx * my, 0);
    vector<double> sr_init_c(N, 0); vector<double> sr_init_d(N, 0);
    vector<double> sr_out_c(N, 0); vector<double> sr_out_d(N, 0);
    ...
    sr_out_c = sr_init_c;
    vector<double> rtcamp(N, 0); vector<double> rtdif(N, 0);
    
    // write initial camp & dif concentration
    double center = margin_x + 9 * gap;
    double scale = r * 8 / 8;
    vector<vector<double> > ruc(mx, vector<double>(my, 0));
    for (int j = 0; j < mx; j++){
        for (int k = 0; k < my; k++){
            int pos = k * mx + j; 
            if (x[j] >= X[0][0] && x[j] <= X[47][0] 
            && y[k] >= X[0][1] && y[k] <= X[47][1]){
    ...
    vector<double> ux(N, 0); vector<double> uy(N, 0); // partial derivative
    ...
    for (int i = 0; i < N; i++){
        double cellP[2] = {0,0};
        cellP[0] = Xmod[i][0]; cellP[1] = Xmod[i][1]; // the i-th cell's location
        double dist = Lx;
        int k_clo = 0; int j_clo = 0;
        for (int k = 0; k < my; k++){
            for (int j = 0; j < mx; j++){
    ...
    vector<vector<vector<vector<double>>>> Sc(N, vector<vector<vector<double>>>(N, vector<vector<double>>(Nc, vector<double>(2, 0)))); 
    vector<vector<vector<double>>> Tc(N, vector<vector<double>>(N, vector<double>(Nc, 0))); 

    vector<double> RangeC(N, rangeC);
    gc_para.rangeC = RangeC; 
    cad.gen_c_init(X, gc_para, Sc, nc, Tc, LB);

    // initialize all the integrins -----------------------------------
    vector<int> ni(N, 0);
    vector<vector<vector<double>>> Si(N, vector<vector<double>>(Ni, vector<double>(2, 0)));
    vector<vector<double>> Ti(N, vector<double>(Ni, 0)); 
    gi_para.argC = ArgC;
    intg.gen_i_init(X, gi_para, Si, ni, Ti, LB);

    // write initialization for motion --------------------------------
    // write initial nc
    for (int i = 0; i < N; i++){
        for (int j = 0; j < N; j++){
            *foutm << nc[i][j] << " ";
    ...
    *foutm << "\n";
    // write initial cells and cadherins locations
    int Tnc_init = pre_count(N-1,N,nc,N);
    N_Vector init_cond_init; double init_cond_array_init[10000];
    ...
    for (int i = 0; i < (N+Tnc_init)*2; i++){
        if (i%2 == 0){
            *foutm << NV_Ith_S(init_cond_init,i) << " " 
            << NV_Ith_S(init_cond_init,i+1) << "\n";
        }
    }
    ...
    *******************************************************************************/
    ...
    while (time_count < T_MAX){
        ...
        N_Vector init_cond; double init_cond_array[10000];
        ...
        if (t1 == 100){ // if no entry in Tc is positive
            t1 = 0;
            cout << "no entry in Tc is positive" << "\n";
        }
        cout << "t1 = " << t1 << "\n";

        // the most recent attempt to form a new cadherin
        ...
        if (t2 == 100){ // if no entry in Tgc_left is positive
            t2 = 0;
            cout << "no entry in Tgc_left is positive" << "\n";
        }
        cout << "t2 = " << t2 << "\n";

        // the integrin that wears out the fastest
        double t3 = 100;
        for (int i = 0; i < N; i++){
            for (int k = 0; k < Ni; k++){
                if (Ti[i][k] > 0 && Ti[i][k] < t3){
                    t3 = Ti[i][k];
                }
            }
        }
        if (t3 == 100){ // if no entry in Ti is positive
            t3 = 0;
            cout << "no entry in Ti is positive" << "\n";
        }
        cout << "t3 = " << t3 << "\n";

        // the most recent attempt to form a new integrin
        vector<double> Tgi_left(Tgi_len, 0);
        for (int i = 0; i < Tgi_len; i++){
            Tgi_left[i] = Tgi[i] - time_count;
        }
        double t4 = 100;
        for (int i = 0; i < Tgi_len; i++){
            if (Tgi_left[i] > 0 && Tgi_left[i] < t4){
                t4 = Tgi_left[i];
            }
        }
        if (t4 == 100){ // if no entry in Tgi_left is positive
            t4 = 0;
            cout << "no entry in Tgi_left is positive" << "\n";
        }
        cout << "t4 = " << t4 << "\n";

        // determine the next step: remove/add a cadherin/integrin
        double t0 = 100;
        if (t1 > 0 && t1 < t0){
            t0 = t1;
        }
        ...
        // if no more attempt or no attachment is left
        if (t0 == 100){
            cout << "t0 = 0" << "\n";
            break;
        }
        ...
        *foutm << Tnc << "\n";
        ...
        // wirte ni for this generation
        for (int i = 0; i < N; i++){
            *foutm << ni[i] << "\n";
        }

        // write how long this fixed group of cadherins & integrins last
        *foutm << t0 << "\n";  

        // write i-sites (don't move) for this generation
        for (int i = 0; i < N; i++){
            for (int k = 0; k < ni[i]; k++){
                for (int l = 0; l < 2; l++){
                    *foutm << Si[i][k][l] << " ";
        ...
        if (time_count >= 20){
            ...
            for (int ti = 0; ti < 100; ti++){
                ...
                ******************************************************************/
                ...
                if (ti == 0){ // the very first initialization
                    flag = CVodeInit(cvode_mem, f_rhs, tstart, init_cond); 
                }
                else{
                ...
                ******************************************************************/
                ...
                // dif equations --------------------------------------------
                vector<double> tdif(N, 0); tdif = rtdif;
                vector<double> ud_init(mx * my, 0); ud_init = ud;
                ...
                sr_init_d = sr_out_d;
                ADI_nonlin(ud_init, sr_init_d, tstart, tend, X, LB,
                tdif, PDpara, FDpara, ud, rtdif, sr_out_d, pulse_marker_d);

                // camp equations -------------------------------------------
                vector<double> tcamp(N, 0); tcamp = rtcamp;
                vector<double> uc_init(mx * my, 0); uc_init = uc;

                // find the location of the pacemaker
                // always let the frontmost red cell be the source
                ...
                for (int j = 0; j < mx; j++){
                    for (int k = 0; k < my; k++){
                        ...
                        double tmp_dist = 0;
                        for (int l = 0; l < 2; l++){
                            tmp_dist += (src_cell[l] - gridP[l]) 
                                      * (src_cell[l] - gridP[l]);
                        }
                        tmp_dist = sqrt(tmp_dist);
                        if (tmp_dist < dist){
                            dist = tmp_dist;
                            src_loc_c[0] = j; src_loc_c[1] = k;
                        }
                    }
                }
                FCpara.src_loc = src_loc_c;
                sr_init_c = sr_out_c;
                ADI_nonlin(uc_init, sr_init_c, tstart, tend, X, LB, 
                tcamp, PCpara, FCpara, uc, rtcamp, sr_out_c, pulse_marker_c);
                ...
                ******************************************************************/
                ...
                if (time_count >= Tlb[lb_check] - int_fdb_t 
                    && time_count <= Tlb[lb_check]){ 
                    // concentrations at cell locations
                    vector<double> ruc_x(N, 0); vector<double> rud_x(N, 0);
                    for (int i = 0; i < N; i++){
                        double cellP[2] = {0,0};
                        cellP[0] = Xmod[i][0]; cellP[1] = Xmod[i][1];
                        double dist = Lx;
                        int k_clo = 0; int j_clo = 0;
                        for (int k = 0; k < my; k++){
                            for (int j = 0; j < mx; j++){
                    ...
                    for (int i = 0; i < N; i++){
                        Feedback_pt[i] += ( pow(rud_x[i],exp_d) / 
                                            (pow(a0,exp_d) + pow(rud_x[i],exp_d)) ) 
                                        * ( pow(b1,exp_c) / 
                                            (pow(b1,exp_c) + pow(ruc_x[i],exp_c)) ) 
                                        * timestep / int_fdb_t;
                        Feedback_tp[i] += ( pow(b0,exp_d) / 
                                            (pow(b0,exp_d) + pow(rud_x[i],exp_d)) ) 
                                        * ( pow(ruc_x[i],exp_c) / 
                                            (pow(a1,exp_c) + pow(ruc_x[i],exp_c)) ) 
                                        * timestep / int_fdb_t;
                    }
                }
            ...
            ******************************************************************/
            ...
            for (int i = 0; i < (N+Tnc)*2; i++){
                if (i%2 == 0){
                    *foutm << NV_Ith_S(init_cond,i) << " " 
                    << NV_Ith_S(init_cond,i+1) << "\n";
                }
            }
            for (int j = 0; j < mx; j++){
                for (int k = 0; k < my; k++){
            ...
            for (int i = 0; i < N; i++){
                for (int j = i+1; j < N; j++){
                    ...
                    for (int k = startP; k < endP; k++){
                        for (int l = 0; l < 2; l++){
                            Sc[i][j][count][l] = NV_Ith_S(init_cond, 2 * k + l);
                        }
                        count += 1;
                    }
                }
            ...
            ******************************************************************/
            ...
            if (time_count >= Tlb[lb_check]){ 
                ...
                for (int i = 0; i < N; i++){
                    if (LB[i] == 1 && Feedback_pt[i] >= tsh_PT 
                        && Feedback_tp[i] < tsh_TP){
                        triggerState[i] = 1;
                        vector<int> diff_weights;
                        diff_weights.push_back(1);
                        diff_weights.push_back(19);
                        std::discrete_distribution<> diff(diff_weights.begin(), 
                                                          diff_weights.end());
                        LB[i] = diff(generator);
                        if (LB[i] == 0){
                    ...
                    else if (LB[i] == 0 && Feedback_pt[i] < tsh_PT 
                            && Feedback_tp[i] >= tsh_TP){
                        triggerState[i] = 1;
                        vector<int> diff_weights;
        ...
        ******************************************************************/
        ...
        // execute "action"s rc ac ri ai ----------------------------------
        if (action == "rc"){
            // find the cadherin to be removed
            int tbr_i = -1; int tbr_j = -1; int tbr_k = -1;
            for(int i = 0; i < N; i++){
                for (int j = i+1; j < N; j++){
                    for (int k = 0; k < nc[i][j]; k++){
                        if (Tc[i][j][k] ==  t1){
                            tbr_i = i; tbr_j = j; tbr_k = k;
                        }
                    }
                }
            }
            // remove the cadherin if it was not already removed
            if (tbr_i != -1 && tbr_j != -1 && tbr_k != -1){
                int tbr_idx[3] = {tbr_i, tbr_j, tbr_k};
                cout << "Remove the cadherin " << Sc[tbr_i][tbr_j][tbr_k][0] << 
                " , " << Sc[tbr_i][tbr_j][tbr_k][1] << " between cell " 
                << tbr_i << " and cell " << tbr_k << "\n";
                cad.removeC(Sc, nc, Tc, tbr_idx, N, Nc);
            }
            // advance Tc by t0
            for (int i = 0; i < N; i++){
            ...
            if (count_nonzero != 0){ // if there exists some new cadherins
                // randomly select one cadherin
                double Psample = 0;
                Psample = *select_randomly(pullout.begin(), pullout.end());

                // find the cadherin
                int tba_i = 0; int tba_j = 0; int tba_k = 0;
                for (int i = 0; i < N; i++){
                    for (int j = i+1; j < N; j++){
                        for (int k = 0; k < Nc; k++){
                            if (nTc[i][j][k] == Psample){
                                tba_i = i; tba_j = j; tba_k = k;
                                cout << "the new cadherin: " 
                                << nSc[tba_i][tba_j][tba_k][0] << " "
                                << nSc[tba_i][tba_j][tba_k][1] << "\n";
                            }
                        }
                    }
                }
                // add between cell i and cell j
                int cidx_add[2] = {tba_i, tba_j};  
                // coordiates of the new cadherin
                double ccor_add[2] = {nSc[tba_i][tba_j][tba_k][0], 
                                      nSc[tba_i][tba_j][tba_k][1]};
                // duration of the new cadherin
                double ctime_add = nTc[tba_i][tba_j][tba_k]; 
                cout << "Add a new cadherin " << ccor_add[0] << " , " << ccor_add[1]
            ...
            // remove the i-site if it was not already removed before
            if (tbr_i != -1 && tbr_k != -1){
                int tbr_idx[2] = {tbr_i, tbr_k};
                cout << "Remove the i-site " << Si[tbr_i][tbr_k][0] << " , " 
                << Si[tbr_i][tbr_k][1] << " between cell " << tbr_i 
                << " and the substrate " << "\n";
                intg.removeI(Si, ni, Ti, tbr_idx, N, Ni);
            }
            // advance Ti by t0
            for (int i = 0; i < N; i++){
                for (int k = 0; k < ni[i]; k++){
            ...
            intg.gen_i(X, gi_para, nSi, nni, nTi, LB);

            // weighted random choice of which cell to form a new integrin
            vector<int> closecells;
            vector<int> weights;
            int weight_sum = 0;
            for (int i = 0; i < N; ++i){
                if (nni[i] > 0){   
                    closecells.push_back(i); // which cell has new integrins
                    // the weight of this cell (depends on how many integrins it has)
                    int weight = (Ni - ni[i]) * (Ni - ni[i]);   
                    weights.push_back(weight);
                    weight_sum += weight;
                }
            }
            if (closecells.size() > 0 && weight_sum != 0){
                std::discrete_distribution<> d(weights.begin(), weights.end());
                int target_weight = d(generator);  // which weight was chosen
                int target_addi = closecells[target_weight]; // which cell was chosen

                // randomly select one i-site among the target cell's integrins
                // all nonzero entries in nTi[target_addi]
                vector<double> pullout = {}; 
                int count_nonzero = 0;
                for (int k = 0; k < Ni; k++){
                    if (nTi[target_addi][k] > 0){
                        double nonzero_select = nTi[target_addi][k];
                        pullout.push_back(nonzero_select);
                        count_nonzero += 1;
                    }
                }
                if (count_nonzero == 0){
                    cout << "no entry in nTi is positive" << "\n";
                }
                else{
                    // randomly select one integrin
                    double Psample = 0;
                    Psample = *select_randomly(pullout.begin(), pullout.end());

                    // find the i-site
                    int tba_i = 0; int tba_k = 0;
                    for (int i = 0; i < N; i++){
                        for (int k = 0; k < Ni; k++){
                            if (nTi[i][k] == Psample){
                                tba_i = i; tba_k = k;  // find the integrin
                            }
                        }
                    }

                    // coordiates of the new integrin
                    double icor_add[2] = {nSi[tba_i][tba_k][0], 
                                          nSi[tba_i][tba_k][1]}; 
                    // duration of the new integrin
                    double itime_add = nTi[tba_i][tba_k]; 
                    cout << "Add a new i-site " << icor_add[0] << " , " 
                    << icor_add[1] << " for cell " << tba_i << " with attach time " 
                    << itime_add << "\n";
                    intg.addI(Si, ni, Ti, tba_i, icor_add, itime_add, N, Ni);
                }
            }
            else{
                cout << "no new i-site is formed" << "\n";
            }
        }
        ...
        /*****************************************************************
                            Restore attachment sites
                              (avoid falling apart)            
        ******************************************************************/
        if (time_count >= 1){  // after the initial noise goes away
            // retore cadherins ------------------------------------------
            for (int i = 0; i < N; i++){
                // check total # of cadherins for cell i
                int Tnc_single = 0;
                for (int j = 0; j < N; j++){
                    Tnc_single += nc[i][j];   // sum up the i-th row in nc
                }
                // if the total # of cadherins of cell i is below mNc
                if (Tnc_single < mNc){   
                    ...
                    if (cand_count != 0){ 
                        // try (mNc - Tnc_single) times to form cadherins
                        for (int k = 0; k < mNc - Tnc_single; k++){ 
                            ...
                            if (LB[i] == 0 && LB[j] == 0){ // TT
                                ...
                                std::discrete_distribution<> d(prob_weights.begin(), 
                                                               prob_weights.end());
                                prob = d(generator);
                            }
                            else if (LB[i] == 1 && LB[j] == 1){ // PP
                                prob_weights.push_back(1);
                                prob_weights.push_back(19);
                                std::discrete_distribution<> d(prob_weights.begin(), 
                                                               prob_weights.end());
                            ...
                            if (prob == 0){ 
                                int cidx_add[2] = {i, j};
                                double ccor_add[2] = {0,0}; double ctime_add = 0;
                                cad.gen_c_single(X, i, j, gc_para, ccor_add, 
                                                 ctime_add, LB);
                                if (ccor_add[0] != 0 && ccor_add[1] != 0){
                                    cout << "Restore a new cadherin " << ccor_add[0]
                                    << " , " << ccor_add[1] << " between cell " << i 
                                    << " and cell " << j << "\n";
                                    cad.addC(Sc, nc, Tc, cidx_add, ccor_add, 
                                             ctime_add, N, Nc);
                                }
                            }
                        }
            ...
            for (int i = 0; i < N; i++){
                if (X[i][1] <= r + Li){
                    ...
                    for (int k = 0; k < mNi - ni[i]; k++){ 
                        ...
                        if (prob == 0){ 
                            int tba_i = i;
                            double icor_add[2] = {0,0}; double itime_add = 0; 
                            intg.gen_i_single(X, tba_i, gi_para, icor_add, 
                                              itime_add, LB);
                            if (icor_add[0] != 0){
                                cout << "Restore a new i-site " << icor_add[0] 
                                << " , " << icor_add[1] << " for cell " << tba_i 
                                << " with attach time " << itime_add << "\n";
                                intg.addI(Si, ni, Ti, tba_i, icor_add, 
                                          itime_add, N, Ni);
                            }
                        }
                    }
        ...
        CVodeFree(&cvode_mem);
    } // end of one generation for a fixed group of cadherins

    // free memory  --------------------------------------------------------
    SUNContext_Free(&sunctx);
    foutm->close(); foutc->close(); foutd->close();
    delete foutm; delete foutc; delete foutd;
...
void Id_i(int ni, double* xi, vector<vector<double> >& sio, double r, double L, vector<double>& Phi){
    for (int i = 0; i < ni; i++){
        Phi[i] = 0;
    }
    for (int k = 0; k < ni; k++){
        double d = 0; double norms = 0;
        for (int l = 0; l < 2; l++){
            d += (xi[l] - sio[k][l]) * (xi[l] - sio[k][l]);
            norms += sio[k][l] * sio[k][l];
        }
        d = sqrt(d);
        if (d >= r && d <= L + r && norms != 0){
            Phi[k] = 1;
        }
    }
}
...
void f_red(int nij, double alpha, double r, double lc, double nl, double* x, vector<vector<double> >& s, vector<vector<double> >& F)
{
    ...
    for (int k = 0; k < nij; k++) {
        ...
        d = sqrt(d);
        if (x[0] >= s[k][0]){ // if the cadherin is formed in the back
            for (int l = 0; l < 2; l++) {
                F[k][l] = alpha * (d - nl * (r + lc)) * (x[l] - s[k][l]) / d;
            }
        }
        else if (x[0] < s[k][0]){ // if the cadherin is formed in the front
            if (d > r + lc) { 
                for (int l = 0; l < 2; l++) {
                    F[k][l] = alpha * (d - r - lc) * (x[l] - s[k][l]) / d;
                }
...
void f1_red(double alpha, double r, double lc, double nl, double* x, double* y, double* F) 
{
    ...
    d = sqrt(d);
    if (x[0] <= y[0]){ // when the cadherin is ahead of the cell
        if (d > r + lc) {
            for (int l = 0; l < 2; l++) {
                F[l] = alpha * (d - r - lc) * (x[l] - y[l]) / d;
            }
    ...
    else if (x[0] > y[0]){ // when the cadherin is behind the cell
        for (int l = 0; l < 2; l++) {
...
int f_rhs(double t, N_Vector y, N_Vector ydot, void* user_data) {
    ...
    double r = mpara->r; double lc = mpara->lc; double Lc = mpara->Lc;
    double li = mpara->li; double Li = mpara->Li;
    double nl = mpara->nl;
    double alphaB = mpara->alphaB; double alphaS = mpara->alphaS;
    vector<double> AlphaM(3,0); AlphaM = mpara->AlphaM;
    double mu = mpara->mu; double nu = mpara->nu;
    vector<double> AlphaC(2,0); vector<double> AlphaI(2,0);
    AlphaC = mpara->AlphaC; AlphaI = mpara->AlphaI;
    vector<int> LB(N, 0); LB = mpara->label;
    vector<vector<int> > nc(N, vector<int>(N, 0)); nc = mpara->nc;
    vector<int> ni(N, 0); ni = mpara->ni;
    vector<vector<vector<double> > > Si(N, vector<vector<double>>(Ni, vector<double>(2, 0))); Si = mpara->Si;
    
    // total number of cadherins
    int Tnc = pre_count(N-1,N,nc,N);
    // copy current value y(t) N_Vector --> V(t) array   
    vector<double> V((N + Tnc) * 2, 0);
    for (int i = 0; i < (N + Tnc) * 2; i++) {
        V[i] = NV_Ith_S(y, i);
    }
    ...
    // Mc encodes forces due to cell-cell connection (cadherins)
    ...
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < N; j++) {
            ...
            int nc_ij = nc[i][j]; // number of cadherins between i and j
            double Vi[2]; double Vj[2];
            for (int l = 0; l < 2; l++) {
                Vi[l] = rV[i][l];
                Vj[l] = rV[j][l];
            }

            if (nc_ij != 0) { // if there exists cadherins between i and j
                // grab all the cadherins between i and j
                // Create a vector containing n vectors of size m.
                vector<vector<double>> Sc(nc_ij, vector<double>(2, 0));
                int startP = N + pre_count(i,j,nc,N);
    ...
    // Mi encodes forces due to substrate connection (i-sites)
    vector<vector<double> > Mi(N, vector<double>(2, 0)); 

    for (int i = 0; i < N; i++) {
        // check cell type and determine alphaI
        double alphaI = 0;
        if (LB[i] == 0){
            alphaI = AlphaI[0];
        }
        ...
        int ni_i = ni[i]; // number of i-sites formed by cell i
        double Vi[2];
        for (int l = 0; l < 2; l++) {
            Vi[l] = rV[i][l];
        }

        if (ni_i != 0) { // if there exists i-sites formed by cell i
            // grab all the i-sites formed by cell i
            vector<vector<double> > si(Ni, vector<double>(2, 0));
            for (int k = 0; k < Ni; k++) {
                for (int l = 0; l < 2; l++) {
                    si[k][l] = Si[i][k][l];
                }
            }
            // the forces due to i-sites
            vector<double> Phif_i(ni_i, 0);
            vector<vector<double> > Fi(ni_i, vector<double>(2, 0)); 
            Id_i(ni_i, Vi, si, r, Li, Phif_i);
            f(ni_i, alphaI, r, li, Li, Vi, si, Fi);
            double dot_Phi_F_i[2] = { 0,0 };
            for (int l = 0; l < 2; l++) {
                for (int k = 0; k < ni_i; k++) {
                    dot_Phi_F_i[l] += Phif_i[k] * Fi[k][l];
                }
            }
            for (int l = 0; l < 2; l++) {
                Mi[i][l] = dot_Phi_F_i[l];
            }
        }
    }

    // G encodes body forces between cells
    vector<vector<vector<double> > > G(N, vector<vector<double> >(N, vector<double>(2, 0)));
    ...
    for (int i = 0; i < N; i++) {
        ...
        for (int l = 0; l < 2; l++) {
            dVdt[i][l] = - sum_Mc[l] / mu - Mi[i][l] / mu + sum_G[l] / mu 
                         + B[i][l] / mu + W[i][l] / mu;
        }
    }
    ...
    for (int i = 0; i < N; i++) {
        for (int j = i + 1; j < N; j++) {
            // check cell type and determine alphaC
            double alphaC_1 = 0; double alphaC_2 = 0;
            if (LB[i] == 0 && LB[j] == 0){
                alphaC_1 = AlphaC[0]; alphaC_2 = AlphaC[0];
            }
            else if (LB[i] == 1 && LB[j] == 1){
                alphaC_1 = AlphaC[1]; alphaC_2 = AlphaC[1];
            }
            else if (LB[i] == 0 && LB[j] == 1){
                alphaC_1 = AlphaC[0]; alphaC_2 = AlphaC[1];
            }
            else if (LB[i] == 1 && LB[j] == 0){
                alphaC_1 = AlphaC[1]; alphaC_2 = AlphaC[0];
            }
            ...
            for (int k = startP; k < endP; k++) {
                double Vk[2];
                for (int l = 0; l < 2; l++) {
                    Vk[l] = rV[k][l]; // cadherins between i and j
                }
                double Fi[2]; double Fj[2];
                if (LB[i] == 0 && LB[j] == 1){
...
void gen_init(int N, vector<vector<double> >& X, vector<vector<int> >& nc, vector<vector<vector<vector<double> > > >& Sc, double* cc_array){
    ...
    // based on the current cell locations X and cadherins locations Sc
    
    // total number of cadherins
    int Tnc = pre_count(N-1,N,nc,N);

    // pack locations of cell centers and cadherins
    vector<vector<double> > Tmpx(N + Tnc, vector<double>(2, 0));
    for (int i = 0; i < N; i++) {
        for (int l = 0; l < 2; l++) {
            Tmpx[i][l] = X[i][l];
        }
    ...
    for (int i = 0; i < N; i++) {
        for (int j = i+1; j < N; j++){
            ...
            for (int k = startP; k < endP; k++){
                for (int l = 0; l < 2; l++) {
                Tmpx[k][l] = Sc[i][j][count][l]; // some of these cadherins are zero
                }
...
void ADI_nonlin(vector<double>& u_init, vector<double>& sr_init, double tstart, double tend, vector<vector<double> >& X, vector<int> label, vector<double>& tchem, PDEpara CKpara, NLpara Fpara, vector<double>& u, vector<double>& rtchem, vector<double>& sr_out, vector<int>& pulse_marker){
    ...
    int N = CKpara.N; 
    double Lx = CKpara.Lx; double Ly = CKpara.Ly;
    int Nx = CKpara.Nx; int Ny = CKpara.Ny;
    double mindt = CKpara.mindt; 
    double sigma = CKpara.sigma; double tol = CKpara.tol;

    double baseT = CKpara.baseT; double baseP = CKpara.baseP;
    double tshT = CKpara.tshT; double tshP = CKpara.tshP;
    double Tt = CKpara.Tt; double Tp = CKpara.Tp;
    double slopT = CKpara.slopT; double slopP = CKpara.slopP;

    double T = tend - tstart;
    double dx = Lx / Nx; double dy = Ly / Ny;

    // determine time step 
    int Nt = 0; double dt = 0;
    if (T < mindt){
        Nt = 1; dt = T;
    }
    else{
    ...
    // parameters
    double rx = (sigma * dt) / (dx * dx); double thetax = sigma / (dx * dx);
    double ry = (sigma * dt) / (dy * dy); double thetay = sigma / (dy * dy);
    ...
    for (int n = 0; n < Nt; n++){
        ...
        /***************************************************************************
                                        ADI method
        ***************************************************************************/
        int INFO = 0; int NRHS = 1;
        int orderAx = Nx - 1; int LDBx = Nx - 1;
        int orderAy = Ny - 1; int LDBy = Ny - 1;

        // predictor step: from u_{n-1} obtain u_{n-1/2} -------------------------
        vector<vector<double> > UH(mx, vector<double>(my, 0));
        vector<double> rhs1(Nx - 1, 0);
        float RHS1[NX-1] = {0};            
        for (int k = 1; k < Ny; k++){
            for (int j = 1; j < Nx; j++){
                RHS1[j-1] = ry * U[j][k-1] / 2 + (1 - ry) * U[j][k] 
                          + ry * U[j][k+1] / 2 + FF[j][k];
            }
        ...
        for (int j = 1; j < Nx; j++){
            for (int k = 1; k < Ny; k++){
                RHS2[k-1] = rx * UH[j-1][k] / 2 + (1 - rx) * UH[j][k] 
                          + rx * UH[j+1][k] / 2 + FF[j][k];
            }
...
void nonlin_F(vector<double>& upre, vector<double>& sr, vector<vector<double> >& X, vector<int>& label, NLpara para, vector<double>& x, vector<double>& y, vector<double>& F){
    ...
    int N = para.N; int Nx = para.Nx; int Ny = para.Ny;
    int K = para.K; double Lx = para.Lx; double Ly = para.Ly;
    double src = para.src;
    vector<int> src_loc{0,0}; src_loc = para.src_loc;
    double Vc = para.Vc; double V0 = para.V0;
    double gm6_t = para.gm6_t; double gm6_p = para.gm6_p;
    double gm7_t = para.gm7_t; double gm7_p = para.gm7_p;
    double gm8 = para.gm8; double gm9 = para.gm9;

    double dx = Lx / Nx; double dy = Ly / Ny;
    double mx = Nx + 1; double my = Ny + 1;
    double mu = 0;
    double sigma = dx; 
    if (dy > dx){
        sigma = dy;
    ...
    for (int k = 0; k < my; k++){
        for (int j = 0; j < mx; j++){
            ...
            for (int i = 0; i < N; i++){
                ...
                dist = sqrt(dist);
                double delta = exp(-(dist - mu)*(dist - mu) / (2*sigma*sigma))
                             / (sigma * sqrt(2*pi));
                int position = k * mx + j;
                double coeff = K * Vc * delta / V0;
                if (label[i] == 0){
                    F[position] += coeff * ( sr[i] - gm7_t * upre[position] 
                                             / (upre[position] + gm6_t) );
                }
                else if (label[i] == 1){
                    F[position] += coeff * ( sr[i] - gm7_p * upre[position] 
                                             / (upre[position] + gm6_p) );
                }
            }
        }
