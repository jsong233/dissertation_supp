...
void Cadherin::gen_c(vector<vector<double> >& X, GCpara gc_para, vector<vector<vector<vector<double> > > >& Sc, vector<vector<int> >& nc, vector<vector<vector<double> > >& Tc, vector<int>& LB){
    ...
    int N = gc_para.N; int Nc = gc_para.Nc;
    ...
    vector<vector<vector<vector<double> > > >  sc(N, vector<vector<vector<double> > >(N,vector<vector<double> >(Nc, vector<double>(2,0)))); // double sc[N][N][Nc][2];      // intermediate possible c-sites
    ...
    for (int i = 0; i < N; i++){
      for (int j = 0; j < N; j++){
          ...
          if (j != i && d <= 2 * r + 2 * Lc){
                ...
                for (int k = 0; k < Nc; k++){  // Nc[i] attempts for cell i
            ...
            for (int k = 0; k < count; k++){
                if (k < nnc[i][j]){
                    ...
                    for (int l = 0; l < 2; l++){
                        ...
                        Sc[i][j][k][l] = sc[j][i][posLeft][l]; // modified on Nov03
                    }
                }
            }
...
void Cadherin::gen_c_init(vector<vector<double> >& X, GCpara gc_para, vector<vector<vector<vector<double> > > >& Sc, vector<vector<int> >& nc, vector<vector<vector<double> > >& Tc, vector<int>& LB){
  ...
  int N = gc_para.N; int Nc = gc_para.Nc; Nc = Nc / 2;
  // vector<int> Nc(N, 0); Nc = gc_para.Nc; 
...
void Cadherin::gen_c_single(vector<vector<double> >& X, int tba_i, int tba_j, GCpara gc_para, double* Sc, double& Tc, vector<int>& LB){
    ...
    if (d <= 2 * r + 2 * Lc){
        ...
        // if the angle between b and delta <= pi/8
        // and the candidate cadherin is within the outreach range of cell j
        if (dot_b_delta/(norm_b * norm_delta) >= cos(pi/8)  && dis_to_j <= r + Lc){ // change to cos(pi/8) on Jul29
            for (int l = 0; l < 2; l++){
                sc[0][l] = sc_cand[l];
            }
        }
