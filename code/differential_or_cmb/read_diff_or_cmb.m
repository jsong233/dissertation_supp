...
sampleIter = length(H);
OR = [0,1,2,3,4,5]; % parameter loop
orIter = length(OR);
RP_cmb = zeros(sampleIter, STEP, orIter);
RP_avg = zeros(STEP, orIter);
AR_cmb = zeros(sampleIter, STEP, orIter);
AR_avg = zeros(STEP, orIter);


for orselect = 1:orIter
    % the or parameter
    or = OR(orselect);
    
    % average over different initilizations under this or
    for hselect = 1:sampleIter
        
        h = H(hselect);
        sprintf("or loop = %d, h loop = %d", or, h)
        t = []; RP = []; AR = [];

        % open file
        filename = "motion" + h + "_or" + or + ".dat";
        ...
        % save vectors
        outputname = "output" + h + "_or" + or + ".mat";
        ...
        for i = 1:STEP
            ...
            [~,j] = min(abs(t - Comp)); 
            RP_cmb(hselect,i,orselect) = RP(j);
            AR_cmb(hselect,i,orselect) = AR(j);
        end
    end
    
    % average over sample iterations
    for i = 1:STEP
        RP_avg(i,orselect) = mean(RP_cmb(:,i,orselect));
        AR_avg(i,orselect) = mean(AR_cmb(:,i,orselect));
    end
end
...
figure(1)
for k = 1:orIter
    eb = shadedErrorBar(T,RP_cmb_merge(:,:,k),{@mean,@std},'lineprops',...
...
for k = 1:orIter
    h(k) = plot(T,RP_avg_merge(:,k),lineSpec{k},'markersize',8,'Color',color{k});
    hold on;
end
l = legend(h,'$\theta_r = \pi$', '$\theta_r = \pi/2$','$\theta_r = \pi/3$',...
    '$\theta_r = \pi/4$','$\theta_r = \pi/8$','$\theta_r = \pi/16$');
% l = legend(h,'$\theta_r = \pi/4$','$\theta_r = \pi/8$','$\theta_r = \pi/16$',...
%     '$\theta_r = \pi/32$','$\theta_r = \pi/64$');
...
set(gca,'FontSize',24);
set(gca,'FontSize',24);
l = legend(h,'$\theta_r = \pi$','$\theta_r = \pi/2$',...
    '$\theta_r = \pi/4$','$\theta_r = \pi/8$','$\theta_r = \pi/16$');
set(l,'Interpreter','latex')
set(l,'FontSize',28);
set(l,'FontName','Times New Roman');
