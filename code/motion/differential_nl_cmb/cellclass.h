...
struct GCpara
{
  ...
  int Nc_max;            // overall maximum number of c-sites formed between two cells
  vector<int> Nc;        // maximum outreach attempts (pairwise) to form cadherins for each cell
  ...
  vector<double> rangeC; // outreach range
...
struct PDEpara
{
  ...
  double baseT;
  double baseP; // baseal level of outputting chemicals
  double tshT;
  double tshP; // threshold to output chemicals
  double Tt;
  double Tp; // period of chemical output
  double slopT;
  double slopP; // slope of chemical output
};
...
struct NLpara
{
  ...
  double src;
  vector<int> src_loc; // exogenous source and source location
  double gm6_t;
  double gm6_p;
  double gm7_t;
  double gm7_p; // local degradation
  double gm8;
  double gm9; // global degradation
};
